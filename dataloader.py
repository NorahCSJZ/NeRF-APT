...
class Spectrum_dataset(Dataset):
    ...
    def load_data(self):
        ...
        with open(os.path.join(self.gateway_pos_dir)) as f:
            ...
            gateway_orientation = gateway_info['gateway1']['orientation'] # w是实部，x,y,z是虚部，w 和旋转的角度有关，与旋转角的余弦值相关； x,y,z这些分量与旋转轴有关,成比例
    ...
    def gen_rays_spectrum(self, gateway_pos, gateway_orientation):
        ...
        azimuth = torch.linspace(1, 360, self.n_azimuth) / 180 * np.pi  # 依据角度到弧度的转换公式
        ...
        r_d = R @ r_d  # [3, n_rays] 3D direction of rays in world coordinate, 将旋转矩阵 R 应用到射线方向 r_d 上，完成从网关坐标系到世界坐标系的转换
...
class CSI_dataset(Dataset):
    ...
    def __init__(self, datadir, indexdir, scale_worldsize=1):
        ...
        with open(os.path.join(self.bs_pos_dir)) as f:
            ...
            self.n_bs = len(self.bs_pos) if len(self.bs_pos.shape) != 1 else 1
    ...
    def load_data(self):
        ...
        if len(bs_ray_o.shape) == 2:
            if bs_ray_o.shape[0] != 1:
                bs_ray_o = bs_ray_o.t()
        else:
...
class NeWRF_dataset(Dataset):
    def __init__(self, datadir, indexdir, scale_worldsize=1):
        ...
        self.csidata_dir = os.path.join(datadir, 'csidata.npy')
        self.bs_pos_dir = os.path.join(datadir, 'base-station.yml')
        self.rssi_dir = os.path.join(datadir, 'gateway_rssi.csv') # rx接收到的数据
        self.dataset_index = np.loadtxt(indexdir, dtype=int)
        self.beta_res, self.alpha_res = 9, 36  # resulution of rays
        ...
        with open(os.path.join(self.bs_pos_dir)) as f:
            bs_pos_dict = yaml.safe_load(f)
            self.bs_pos = torch.tensor([bs_pos_dict["base_station"]], dtype=torch.float32) #.squeeze() # newrf
            # self.bs_pos = self.bs_pos / scale_worldsize # base location
            self.n_bs = len(self.bs_pos)

        # load CSI data
        csi_data = torch.from_numpy(np.load(self.csidata_dir))  #[N, 8, 52], 52 subcarriers, 8 Number of Transmitting Antennas
        csi_data = self.normalize_csi(csi_data)
        uplink, downlink = csi_data[..., :26], csi_data[..., 26:]
        up_real, up_imag = torch.real(uplink), torch.imag(uplink)
        down_real, down_imag = torch.real(downlink), torch.imag(downlink)
        ...
        self.nn_inputs, self.nn_labels = torch.tensor(self.uplink, dtype=torch.float32), torch.tensor(self.downlink, dtype=torch.float32)
...
class Spectrum_MLP_dataset(Dataset):
    """spectrum dataset class
    """
    def __init__(self, datadir, indexdir, scale_worldsize=1) -> None:
        super().__init__()
...
dataset_dict = {"rfid": Spectrum_dataset, "ble": BLE_dataset, "mimo": CSI_dataset, "newrf": CSI_dataset, "VAE": CSI_dataset, "MLP": CSI_dataset}
